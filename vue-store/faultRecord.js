...
const queryMTS = FaultRecordSTS.mutations.query
const queryATS = FaultRecordSTS.actions.query
const queryGTS = FaultRecordSTS.getters.query
...
const queryActions = {
		...
		try {
			...
			if(httpResult.success) {
				...
				for(var obj of datas) {
					tmpResult.push(Service.changeRecordList(obj,false))
				}
	...
	async [queryATS.loadMore]({
		...
		try {
			...
			if(httpResult.success) {
				...
				var queryResults = []
				for(var obj of datas) {
					queryResults.push(Service.changeRecordList(obj,false))
				}
				//更新列表
	...
	async [queryATS.revokeClick]({
		...
		try {
			console.log("revokeClick---",queryParams)
